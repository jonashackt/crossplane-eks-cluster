...
To use the kuttl-created kind cluster outside of the kuttl testing harness (e.g. for development purposes), simply run:

```shell
export KUBECONFIG="/home/jonashackt/dev/crossplane-eks-cluster/kubeconfig"
```

You should now also be able to use your existing k8s tooling like `k9s` etc.

If you need a working AWS config, to really provision things in your AWS account, you need to create a 'real' AWS config and Secret (since with the kuttl setup, the default is to use a Secret without real authorization):

```shell
# Delete no-access-secret
kubectl delete -n crossplane-system secret aws-creds
...
kubectl apply -f examples/eks/xr.yaml 
```
...
With this Composition we're able to use both pre-defined Compositions `Networking` and `EKSCluster` and thus implement a nested Composite Resource:
...
## Accessing the Crossplane provisioned EKS cluster (or how to propagate the EKS Cluster kubeconfig to the nested Composition)

With Crossplane v2 connection secret handling changed! https://docs.crossplane.io/latest/guides/function-patch-and-transform/#composite-resource-connection-details

> "Function Patch and Transform automatically aggregates connection details from composed resources. Unlike other composition functions, Function Patch and Transform can’t access observed connection details to enable manually composing a Secret resource. Instead, it provides built-in automatic aggregation."

To expose connection details for a composite resource:

    1. Set writeConnectionSecretToRef on each composed resource that produces connection details
    2. Define connectionDetails on each resource to specify which secret keys to include in the aggregated secret
    3. Configure where to write the aggregated connection details secret (see options below)

The function automatically creates a Secret composed resource containing all the aggregated connection details.

--> Here's also a full guide https://docs.crossplane.io/latest/guides/connection-details-composition/


Inside our EKSCluster Composition [apis/eks/composition.yaml](apis/eks/composition.yaml) there a `ClusterAuth` CRD with the needed `connectionDetails` and patches configured:
...
so if you run a `kubectl get secret`, you should find the Crossplane generated Secret containing the EKS cluster's `kubeconfig`:

![](docs/eks-cluster-kubeconfig.png)

The secret is named after the `spec.id`field defined in the XR. In our EKSCluster example [XR](upbound/provider-aws/apis/eks/xr.yaml) we defined
...
Now use the contents `ekskubeconfig` as your kubeconfig by exporting it:

```shell
export KUBECONFIG="/home/jonashackt/dev/crossplane-eks-cluster/ekskubeconfig"
```

If you're on the new context of our Crossplane bootstrapped EKS cluster, check if everything works:

```shell
$ kubectl get nodes
NAME                                         STATUS   ROLES    AGE   VERSION
ip-10-0-0-60.eu-central-1.compute.internal   Ready    <none>   24m   v1.35.3-eks-bbe087e
ip-10-0-1-70.eu-central-1.compute.internal   Ready    <none>   24m   v1.35.3-eks-bbe087e
ip-10-0-2-55.eu-central-1.compute.internal   Ready    <none>   24m   v1.35.3-eks-bbe087e
```

If you want to return to your local kind cluster - or any other cluster - unset the `KUBECONFIG` local env var via:

```shell
unset KUBECONFIG
# or revert back to the kuttl kind cluster
export KUBECONFIG="/home/jonashackt/dev/crossplane-eks-cluster/kubeconfig"
```
...
## Writing the asserts (based on the Crossplane generated CRDs)

> Be sure to have a up-to-date crossplane CLI installation in place - see https://docs.crossplane.io/latest/cli/

With the latest crossplane CLI you the `crossplane render` command available. With it you can testdrive your Compositions using a example XR like this:

```shell
crossplane render examples/eks/xr.yaml apis/eks/composition.yaml crossplane-contrib/function-patch-and-transform/function.yaml
```

Since with Crossplane v2 we're using Functions inside our Compositions, we need to also define them here with the render command.
